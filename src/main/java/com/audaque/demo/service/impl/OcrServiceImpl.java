...
public class OcrServiceImpl implements OcrService {
    ...
    public OcrResponse recognize(MultipartFile image) {
        ...
        try {
            ...
            long elapsed = System.currentTimeMillis() - start;
            log.error("OCR 识别失败：读取图片失败", e);
            return new OcrResponse(false, "读取图片失败: " + e.getMessage(), model, elapsed);
        } catch (Exception e) {
            long elapsed = System.currentTimeMillis() - start;
            log.error("OCR 识别失败：LLM 调用异常", e);
            return new OcrResponse(false, "LLM 调用失败: " + e.getMessage(), model, elapsed);
        }
    }
}
